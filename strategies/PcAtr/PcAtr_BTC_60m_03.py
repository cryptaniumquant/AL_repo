...
sys.path.append("..")
# Собственные классы
from tradingMathematics import indicators
from tradingMathematics import metrics_punkt
from tradingMathematics import tradingTactics
from tradingMathematics import array_operations
...
import numba as nb  # Для ускорения jit (just in time)
import quantstats as qs  # Для расчета финансовых показателей и статистики
import matplotlib.gridspec as gridspec
# import time  # для вызова sleep и т.д.
# import datetime
...
import csv
import io
qs.extend_pandas()  # Вносит в функционал питоновоского экселя (Pandas) дополнительные методы
import warnings
from trailing_stops import trailing_stops
from symbol_params import symbol_params_dict as SSS

# endregion

# region Описание работы стратегии

"""
Анализ параметров и сужение сетки:

    Для каждой стратегии создается своя папка: .csv, .csv, pdf
    Одна большая функция, которая считает сразу все метрики
    Гистограмма с по-месячными доходностями

  1. Сначала прогоняем и отбрасываем:
     - убыточные <0% за год
     - с маленьким кол-вом сделок в год 
    В итоге количество сочетаний сильно уменьшается: 1M -> 200k строк 
    .csv: pnl, dd, trades -> записывать
  2.2. 200k считаем все метрики основные -> .csv с 200k строчек и все метрики
  2.2. .csv  - штрафы - топ стратегий (минимальная сумма штрафов) - 100: 
  pdf отчет, html, jpg | сохранять все а потом будут удаляться остальные

"""
# endregion

# region PcAtr_BTC_60m_03 - главная функция стратегии

# Объявляем функцию, которая на вход принимает свечи и аргументы стратегии
# @nb.njit(parallel=True)  # Превращаем функцию в код на C
def PcAtr_BTC_60m_03(
    ...
    #region устанавливаем характеристики финансового инструмента (symbol) и биржи

    symbol = _symbol # Укажите название инструмента, который собираетесь торговать
    ...
    symbol_params_dict = SSS
    #endregion

    # Функция для получения параметров для заданного инструмента
    def get_symbol_params(symbol):
        '''
        Функция get_symbol_params принимает один аргумент symbol
        Возвращает значение из словаря symbol_params_dict по ключу symbol
        Если ключ symbol не найден, возвращаем None по умолчанию
        '''
    ...
    if symbol_params:
        ...
        print(f"Параметры для инструмента {symbol} не найдены.")
        #Прерываем дальнейшее выполнение функции (стратегии)
        raise KeyError(f"Параметры для инструмента '{symbol}' не найдены в словаре symbol_params_dict.") #прерываем функцию

    strategyName = inspect.currentframe().f_code.co_name # PcAtr_BTC_60m_02" - имя стратегии совпадает с названием функции
    write_to_log: bool = False  # Нужно ли писать логи по умолчанию False
    isOptimization = False  # Находится ли система в процессе оптимизации
    isRealTime = False  # Находится ли система в "боевом" режиме выдачи сигналов аналитиком
    isExecuteOn = True  # Если True продолжаем выполнять программу
    if isExecuteOn == False: return ({'Работа прервана': 0}) # Заглушка для работы системы (применяется при загрузке новых данных)
    ...
    AbsComission_arr = np.zeros(bars_count,
                                dtype=np.float64)  # для хранения уплаченной комиссии (при расчёте рублей на контракт)
    RelComission_arr = np.zeros(bars_count,
                                dtype=np.float64)  # для хранения уплаченной комиссии (при расчёте % от объёма сделки)
    ...
    : int = 0  # первый номер бара, на котором существуют все индикаторы
    ...
    _koeff, _dividerAtr, _periodAtr, _periodEnterPC, maxLeverage = params_to_optimize
    ...
    isFixedMode: bool = False  # Режим расчёта эквити (False - unRealized (зелёное), True - Realized (фиолетовое)
    ...
    # region Проводим расчёт комиссии (только в Лаборатории) - Абсолютной или Относительной

    '''
    if useAbsComission:  # Если используется абсолютная (в пунктах) комиссия
    # = absComission  # устанавливаем комиссию в рублях на контракт
    # Рассчитываем для фин. инструмента комиссию методо AbsComission_calc
    else:  # Если используется относительная (в %) комиссия
    # устанавливаем сколько % комиссии берётся от объёма сделки
    # Устанавливаем стоимость заёмных средств
    # Рассчитываем для фин. инструмента RelComission_calc
    '''
    # endregion
    ...
    high_level_enter_pd = indicators.Highest(High_pd, periodHighEnter)
    high_level_enter_pd = high_level_enter_pd.shift(1)  # сдвигаем индикатор highLevel на один бар вправо (с помощью Panda)
    first_valid_value = max(first_valid_value, periodHighEnter)  # выбираем наибольшее

    low_level_enter_pd = indicators.Lowest(Low_pd, periodLowEnter)
    low_level_enter_pd = low_level_enter_pd.shift(1)  # сдвигаем индикатор highLevel на один бар вправо (с помощью Panda)
    first_valid_value = max(first_valid_value, periodLowEnter)  # выбираем наибольшее

    atr_pd = indicators.ATR_tslab_calc(Close_pd, High_pd, Low_pd, periodAtr)
    first_valid_value = max(first_valid_value, _periodAtr)  # выбираем наибольшее

    # Рассчитываем предварительные стоп-лоссы
    stop_price_long_pd = tradingTactics.stopPriceLong_calc(high_level_enter_pd, low_level_enter_pd, koeff)
    stop_price_short_pd = tradingTactics.stopPriceShort_calc(high_level_enter_pd, low_level_enter_pd, koeff)
    ...
    trailing_for_long_np = np.zeros(Close_np.shape, dtype=np.float64)

    # Пустой массив (контейнер) для хранения трейлинга для сопровождения Short
    trailing_for_short_np = np.zeros(Close_np.shape, dtype=np.float64)
    ...
    if write_to_log:  # Если в параметрах указано, что нужно выводить это значение
        if isRealTime:  # когда находимся в режиме Агента
            logText = f"Cтратегия ({strategyName}) запущена в боевом режиме (режиме Агента)."
        else:
            logText = f"Стратегию ({strategyName}) запущена в режиме Лаборатории (Тестирования)."
            print(logText)
    if write_to_log:
        print(f"Название стратегии: {strategyName}: "
              f"Дата начала торговли: {first_date_string} "
              f"Дата окончания торговли: {last_date_string}\n"
              f"Торговля началась с бара №{first_valid_value}\n"
              f"Если хотите посмотреть сделки - присвойте переменной 'write_to_log' зачение 'True'.")
...
def main():
    ...
    bars_df = pd.read_csv('data/ENAUSDT.csv')
    # print(bars_df)
    ...
    params_to_optimize = [0.25, 21, 896, 108, 98]
